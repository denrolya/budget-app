...
import { Table, Container, Row, Col, Form, FormGroup, Label, Input, Card, Button } from 'reactstrap';
          ...
          <>
            <Container>
              {tuples.map(([f, t]) => (
                ...
                >
                  {f} <i className="ion-ios-swap" aria-hidden /> {t}
                </Button>
              ))}
              <Card body>
                <Form>
                ...
                <>
                  <div className="mb-5">
                    ...
                    <h2>
                      ...
                      <span className="text-muted">{result.slice(result.indexOf('.') + 3)}</span> {CURRENCIES[to]}
                      {result > 1 && 's'}
                    </h2>
                    <p>
                      1 {from} = {exchangeRates[from] / exchangeRates[to]} {to}
                    </p>
                    <p>
                      1 {to} = {exchangeRates[to] / exchangeRates[from]} {from}
                    </p>
                  </div>
                  <Row>
                    <Col>
                          ...
                          <tbody>
                            {EXAMPLES.map((number) => (
                              <tr key={number}>
                                ...
                                <td className="text-center">
                                  {parseFloat(convert(number, from, to).toFixed(2)).toLocaleString()} {to}
                                </td>
                              </tr>
                            ))}
                          </tbody>
                          ...
                          <tbody>
                            {EXAMPLES.map((number) => (
                              <tr key={number}>
                                ...
                                <td className="text-center">
                                  {parseFloat(convert(number, to, from).toFixed(2)).toLocaleString()} {from}
                                </td>
                              </tr>
                            ))}
                          </tbody>
