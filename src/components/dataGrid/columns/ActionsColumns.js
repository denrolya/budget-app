...
const makeDictionary = (values, getKey) =>
  values.reduce((acc, v) => {
    acc[getKey(v)] = v;
    return acc;
  }, {});
...
const ActionColumns = ({ columns }) => {
  ...
  const computeColumns = useCallback(
    (columns, getters) =>
      getters.tableColumns.map((tableColumn) => {
  ...
  const isActionTableCell = ({ tableRow, tableColumn }) =>
    (tableRow.type === Table.ROW_TYPE || tableRow.type === VirtualTable.ROW_TYPE) &&
    tableColumn.type === ACTION_COLUMN_TYPE;

  const isActionTableHeader = ({ tableRow, tableColumn }) =>
    tableRow.type === TableHeaderRow.ROW_TYPE && tableColumn.type === ACTION_COLUMN_TYPE;
